...
internal sealed class ConnectionGroupGrain : IConnectionGroupGrain, IGrainBase
{
    ...
    public ConnectionGroupGrain(
        ...
        [PersistentState("ConnectionGroups", SignalROrleansConstants.SIGNALR_ORLEANS_STORAGE_PROVIDER)]
        IPersistentState<ConnectionGroupGrainState> state)
    {
        _logger = logger;
        _state = state;
    ...
    private Task SendAll([Immutable] InvocationMessage message, IEnumerable<string> connectionIds)
    {
        ...
        return Task.WhenAll(connectionIds.Select(connectionId =>
            _grainFactory.GetClientGrain(_key.HubType, connectionId).Send(message)));
    }
}