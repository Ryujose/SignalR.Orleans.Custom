...
{
    public static HubContext<THub> GetHub<THub>(this IGrainFactory grainFactory) where THub : Hub => new(grainFactory);
    ...
    {
        var key = new ConnectionGroupKey
        {
            GroupId = groupName, HubType = hubName, GroupType = ConnectionGroupType.NamedGroup
        }.ToPrimaryGrainKey();
    ...
    internal static IConnectionGroupGrain GetUserGrain(this IGrainFactory factory, string hubName, string userId)
    {
        var key = new ConnectionGroupKey
        {
            GroupId = userId, HubType = hubName, GroupType = ConnectionGroupType.AuthenticatedUser
        }.ToPrimaryGrainKey();
        return factory.GetGrain<IConnectionGroupGrain>(key);
    }
