...
public static class StreamExtensions
{
    ...
    /// <summary>
    ///     Gets a stream that you can use to write messages that need to be sent to the connection that is connected at that
    ///     server.
    /// </summary>
    ...
    /// <summary>
    ///     Gets a stream that you can listen on to receive a message when the connection with the given
    ///     <paramref name="connectionId" /> has been disconnected.
    /// </summary>
    /// <param name="connectionId">The id of the connection that we are listening for.</param>
    public static IAsyncStream<string> GetClientDisconnectionStream(this IStreamProvider streamProvider,
        string connectionId)
