...
public static class TaskExtensions
{
    ...
    public static Task OrTimeout(this Task task, int milliseconds = DefaultTimeout,
        ...
        [CallerLineNumber] int? lineNumber = null) => OrTimeout(task, new TimeSpan(0, 0, 0, 0, milliseconds),
        ...
        [CallerFilePath] string? filePath = null, [CallerLineNumber] int? lineNumber = null)
    {
        ...
        var completed = await Task.WhenAny(task,
            Task.Delay(Debugger.IsAttached ? Timeout.InfiniteTimeSpan : timeout, cts.Token));
        if (completed != task) throw new TimeoutException(GetMessage(memberName, filePath, lineNumber));
        cts.Cancel();

        await task;
    }

    public static Task<T> OrTimeout<T>(this ValueTask<T> task, int milliseconds = DefaultTimeout,
        [CallerMemberName] string? memberName = null, [CallerFilePath] string? filePath = null,
        [CallerLineNumber] int? lineNumber = null) =>
        OrTimeout(task, new TimeSpan(0, 0, 0, 0, milliseconds), memberName, filePath, lineNumber);

    public static Task<T> OrTimeout<T>(this ValueTask<T> task, TimeSpan timeout,
        [CallerMemberName] string? memberName = null, [CallerFilePath] string? filePath = null,
        [CallerLineNumber] int? lineNumber = null) =>
    ...
    {
        if (!string.IsNullOrEmpty(memberName)) return $"Operation in {memberName} timed out at {filePath}:{lineNumber}";
